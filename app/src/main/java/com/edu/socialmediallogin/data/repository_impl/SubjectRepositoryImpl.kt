...
class SubjectRepositoryImpl(private val apiService: ApiService, private val dao: Dao) :
    SubjectRepository {
    override suspend fun getSubjects(): List<SubjectItem?>? {
        try {
            return apiService.getAllSubjects().result?.map { it }
        } catch (e: Exception) {
            throw Exception(e)
        }
    }
...
//    }
    ...
    override suspend fun insertSubject(
        listOfSubject: List<SubjectEntity>
//        photoUrl: String?,
//        name: String?,
//        description: String?,
//        isIvy: Boolean?
...
//                )
//            )
            dao.insertSubject(subject = listOfSubject)
        } catch (e: Exception) {
